...
use lyric_rpc::task::{DataFormat, DataObject, Language, TaskInfo, TaskState, TaskStateInfo};
...
impl PyLyric {
    ...
    fn submit_task(
        ...
        self.runtime.runtime.block_on(async move {
            lyric
                .submit_task(task_info, env)
                .await
                .map(|state_info| format!("{:?}", state_info))
                .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))
        })
    }

    #[pyo3(name = "submit_task_async_old", signature = (py_task_info, environment_config=None))]
    async fn submit_task_async_old(
        &self,
        py_task_info: PyTaskInfo,
        environment_config: Option<PyEnvironmentConfig>,
    ) -> PyResult<String> {
            ...
            .submit_task_with_callback(task_info, env, move |result| {
                ...
                async move {
                    match result {
                        ...
                        Ok(TaskStateResult::StreamTaskState(stream_state_info)) => {
                            tracing::debug!("Received stream state info: {:?}", stream_state_info);
                            let py_result =
                                PyStreamDataObjectIter::from_async_stream(stream_state_info).await;
                            rt.runtime.spawn_blocking(move || {
                                tracing::debug!("Calling Python callback with stream state info");
                                Python::with_gil(|py| callback.call1(py, (py_result,)))
                            });
                        }
                        Err(e) => {
    ...
    loop {
        tokio::select! {
            ...
            msg = rx_lang_worker.recv() => {
                ...
                match msg {
                    Some(msg) => {
                        _handle_submit_with_type(inner.clone(), runtime.clone(), msg).await?;
                    }
                    None => {
    ...
    let str_res = if let Ok(value) = result.downcast_bound::<PyString>(py) {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            while let Some(item) = futures::executor::block_on(stream.next()) {
                ...
                // s.output = Some(DataObject {
                //     object_id: "".to_string(),
                //     format: DataFormat::Raw as i32,
                //     data: item.as_bytes().to_vec(),
                // });
                // tracing::debug!("Received item from stream: {:?}", item);
                let _ = tx.send(Ok(s));
            }
